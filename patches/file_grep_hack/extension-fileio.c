...
bf_file_grep(Var arglist, Byte next, void *vdata, Objid progr)
{
	...
	Var fhandle;
	const char *real_filename;
	const char *mode = "r-tn";
	file_mode rmode;
	file_type type;
	FILE *f;
	int len;
	
	const char *fmode = file_modestr_to_mode(mode, &type, &rmode);
	const char *pathspec = arglist.v.list[1].v.str;
	const char *real_pathname;
    const char *searchspec = arglist.v.list[2].v.str;
		...
		if (!(curdir = opendir (real_pathname)))
			...
			while ( (curfile = readdir(curdir)) != 0 ) {
				if (strncmp(curfile->d_name, ".", 2) != 0 && strncmp(curfile->d_name, "..", 3) != 0) {
					...
					real_filename = reset_stream(s);
					
					if ((fhandle = file_handle_new(curfile->d_name, type, rmode)).v.num < 0) {
						/* too many open files */
						failed = 1;
						break;
					} else if ((f = fopen(real_filename, fmode)) == NULL) {
						...
						while ((line = file_read_line(fhandle, &len)) != NULL) {
							/* the 0 below means NOT case sensitive */
							if (strindex(line, searchspec, 0) > 0) {
...
register_fileio(void)
{
  ...
  */
  ...
  register_function("file_grep", 2, 2, bf_file_grep, TYPE_STR, TYPE_STR);
